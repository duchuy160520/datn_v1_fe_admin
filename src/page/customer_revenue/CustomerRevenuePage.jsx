...
const CustomerRevenuePage = () => {
    ...
    const handleExport = async (values) => {
        ...
        setLoading(true)
        try {
            const axiosRes = await AxiosApi.postAuthExport(WsUrl.ADMIN_REPORT_REVENUE_BY_CUSTOMER_EXPORT, values)
            console.log("handleExport axiosRes: ", axiosRes);
            if (axiosRes) {
                FileService.saveByteArray(axiosRes, "Doanhthutheokhachhang")
            }
        } catch (e) {
    ...
    const getReport = async () => {
        ...
        setLoading(true)
        try {
            const resData = await AxiosApi.postAuth(WsUrl.ADMIN_REPORT_REVENUE_BY_CUSTOMER, req)
            if (resData) {
                const { data } = resData
        ...
        <div className="container-fluid w-100 reponsive">
            <div className="card shadow mb-4">
                <div className="card-header py-3 d-flex justify-content-between">
                    <h3 className="m-0 font-weight-bold text-primary">Doanh thu theo khách hàng</h3>
                    <div>
                    ...
                    <div className='d-flex justify-content-between align-items-center'>
                        ...
                        <div className='col-2'>
                            <select className='form-control' onChange={handleChangeOrderFilter}>
                                <option value='purchases-asc'>Số lượt mua (thấp - cao)</option>
                                <option value='purchases-desc'>Số lượt mua (cao - thấp)</option>
                                <option value='turnover-asc'>Doanh thu (thấp - cao)</option>
                                <option value='turnover-desc'>Doanh thu (cao - thấp)</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <table className="table table-bordered mt-4" id="dataTable" width="100%" cellSpacing={0}>
                            <thead>
                                <tr className='text-bold text-dark'>
                                    ...
                                    <th className=''>Tên</th>
                                    <th className=''>Điện thoại</th>
                                    <th className=''>Số lượt mua</th>
                            ...
                            <tbody>
                                {
                                    loading ? <HashSpinner /> : revenues && revenues.map((obj, index) => (
                                        <tr key={obj.id}>
                                            ...
                                            <td><Link to={`/user/detail/${obj?.userId}`}>{obj.fullName}</Link></td>
                                            <td>{obj?.phone}</td>
                                            <td>{obj?.purchases}</td>
                                            <td>{obj?.turnover}</td>
                                        </tr>
